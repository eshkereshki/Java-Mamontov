# Лабараторная работа по Java №1 (вариант №7)
...
└── BinTree.java
```
# Лабораторная работа по Java №4 (вариант №7)

**Автор**

*Студент: Мамонтов Станислав Игоревич*
...
1. **Point3D** — класс для представления точки в трёхмерном пространстве.
2. **Box** — класс, описывающий коробку.
3. **Storage** — класс для хранения коллекции коробок.
4. **Filterer** — интерфейс для фильтрации объектов.
5. **Reducer** — интерфейс для объединения элементов.
6. **Collector** — интерфейс для сбора результатов.
7. **Processor** — класс для обработки коллекций.
8. **BoxUtil** — вспомогательный класс с утилитными методами.
9. **Main** — класс для запуска программы.
...
## Задача 1: Работа с точками (Point3D.java)

**Формулировка:**
Создать класс для представления точки в трёхмерном пространстве.

**Описание реализации:**

Класс `Point3D` содержит координаты точки:

* `x`
* `y`
* `z`

Этот класс используется для задания положения объектов в пространстве.

---

## Задача 2: Создание коробок (Box.java)

**Формулировка:**
Создать класс, описывающий коробку, которая имеет размеры и положение в пространстве.

**Описание реализации:**

Класс `Box` хранит информацию о коробке:

* размеры коробки
* положение в пространстве (`Point3D`)

Позволяет создавать объекты коробок и работать с их параметрами.

---

## Задача 3: Хранение коробок (Storage.java)

**Формулировка:**
Реализовать класс для хранения коллекции коробок.

**Описание реализации:**

Класс `Storage` содержит коллекцию объектов `Box` и предоставляет методы для:

* добавления коробок
* хранения объектов
* получения списка коробок

---

## Задача 4: Фильтрация объектов (Filterer.java)

**Формулировка:**
Создать интерфейс для фильтрации элементов коллекции по заданному условию.

**Описание реализации:**

Интерфейс `Filterer` определяет метод, который позволяет проверять объект на соответствие условию фильтрации.

Используется для отбора элементов коллекции.

---

## Задача 5: Агрегация данных (Reducer.java)

**Формулировка:**
Создать интерфейс для объединения элементов коллекции и получения итогового результата.

**Описание реализации:**

Интерфейс `Reducer` используется для:

* объединения значений
* выполнения операций свёртки
* получения итогового результата обработки.

---

## Задача 6: Сбор результатов (Collector.java)

**Формулировка:**
Создать интерфейс для накопления и хранения результатов обработки коллекции.

**Описание реализации:**

Интерфейс `Collector` предназначен для:

* накопления обработанных элементов
* сохранения результатов обработки в структуре данных.

---

## Задача 7: Обработка коллекций (Processor.java, BoxUtil.java)

**Формулировка:**
Реализовать обработку коллекции объектов с использованием созданных интерфейсов.

**Описание реализации:**

Класс `Processor` выполняет обработку коллекции коробок:

* применяет фильтрацию (`Filterer`)
* выполняет объединение элементов (`Reducer`)
* собирает результаты (`Collector`)

Класс `BoxUtil` содержит вспомогательные методы для работы с объектами `Box`.
...
# Лабораторная работа по Java №5 (вариант 7)
...
1. **Fraction / CachedFraction** — работа с дробями и кэшированием вычислений.
2. **Cat / Meowable / Funs** — работа с интерфейсами и подсчётом действий объектов.
3. **ListTasks** — работа со списками и заменой подсписков.
4. **QueueTask** — работа со структурой данных очередь.
5. **GasStationTask** — обработка данных о ценах на бензин.
6. **FileTask** — обработка текста из файла.
7. **StreamPointTask** — работа со Stream API и геометрическими объектами.
8. **StreamNameTask** — обработка данных из файла с использованием Stream API.
9. **Main** — класс для запуска программы.
...
# Задание 1 — Шаблоны (Fraction)

### Формулировка

Создать сущность **Дробь**, которая:

* имеет числитель и знаменатель
* может возвращать вещественное значение
* может возвращать строковое представление дроби
* корректно обрабатывает отрицательные значения
* знаменатель не может быть отрицательным

Также необходимо создать версию дроби, которая **кэширует вычисление вещественного значения**.

### Реализация

Созданы классы:

* `Fraction` — базовый класс дроби
* `CachedFraction` — класс, который кэширует вычисление значения

### Алгоритм

1. Создать интерфейс `Fraction`.
2. Добавить методы:

   * получение вещественного значения
   * установка числителя
   * установка знаменателя
3. Реализовать класс `CachedFraction`.
4. При первом вычислении сохранять результат.
5. При повторном обращении возвращать сохранённое значение.

---

# Задание 2 — Структурные шаблоны (Коты)

### Формулировка

Создать класс **Кот**, который:

* имеет имя
* может мяукать
* выводит сообщение `"Имя: мяу"`

Необходимо определить **количество мяуканий кота**.

Также необходимо реализовать интерфейс для объектов, которые могут мяукать.
...
3. Создать класс `Cat`, реализующий интерфейс.
4. Добавить счётчик мяуканий.
5. Реализовать метод `meow()`.
6. Создать метод `Funs.meowsCare`, который принимает несколько объектов `Meowable`.
7. Вызвать `meow()` для каждого объекта.
...
Создать программу, которая в списке **L** заменяет **первое вхождение списка L1** на **список L2**.

### Алгоритм

1. Получить основной список `L`.
2. Найти индекс первого вхождения списка `L1`.
3. Если подсписок найден:

   * удалить элементы `L1`
   * вставить элементы `L2`.
4. Вывести результат.

---

# Задание 4 — Работа с очередью

### Формулировка

Определить, есть ли в очереди **элемент, равный следующему за ним элементу (по кругу)**.

### Алгоритм

1. Создать очередь.
2. Пройти по всем элементам.
3. Сравнить каждый элемент со следующим.
4. Последний элемент сравнить с первым.
5. Если найдено совпадение — вернуть `true`.

---

# Задание 5 — Автозаправочные станции

### Формулировка

В городе проводится мониторинг цен на бензин марок:

* 92
* 95
* 98

Необходимо определить **сколько АЗС продают бензин дешевле всего** для каждой марки.

### Алгоритм

1. Прочитать список записей о заправках.
...
3. Разделить данные по маркам бензина.
4. Найти минимальную цену для каждой марки.
5. Посчитать количество АЗС с этой ценой.
6. Вывести результат.

---

# Задание 6 — Работа с файлом
...
1. Прочитать файл.
2. Определить набор глухих согласных.
3. Найти их в тексте.
4. Удалить повторяющиеся символы.
5. Отсортировать результат.
6. Вывести символы.

---

# Задание 7 — Stream API (геометрия)

### Формулировка

Дан набор объектов **Point**.

Необходимо:

* удалить точки с одинаковыми координатами
* отсортировать по координате **X**
* сделать координаты положительными
* собрать точки в **ломаную линию**
...
2. Удалить дубликаты.
3. Отсортировать точки по `X`.
4. Преобразовать координаты в положительные значения.
5. Создать объект `Polyline`.

---

# Задание 8 — Stream API (файл имён)
...
1. Прочитать файл.
2. Разделить строки по символу `:`.
3. Удалить записи без номера.
4. Привести имена к формату с заглавной буквой.
5. Сгруппировать имена по номеру.
6. вывести результат.
...
# Лабораторная работа по Java №6 (вариант 7)
...
1. **Invoke / InvokeProcessor / TestClass** — работа с аннотацией `@Invoke`.
2. **Default / DefaultProcessor / DefaultAnnotatedClass** — обработка аннотации `@Default`.
3. **ToString / ToStringProcessor / Person / Mode** — создание строкового представления объекта через аннотацию.
4. **Validate / ValidateProcessor / ValidatedClass** — проверка классов через аннотацию `@Validate`.
5. **Two / TwoProcessor / TwoAnnotatedClass** — обработка аннотации с несколькими параметрами.
6. **Cache / CacheProcessor / CacheAnnotatedClass** — работа с аннотацией кэширования.
7. **Task2JUnit** — тестирование аннотаций с использованием JUnit.
...
src
 ...
 ├ 5
 │  ├ Two.java
 │  ├ TwoAnnotatedClass.java
 │  ├ InvalidTwoAnnotatedClass.java
 │  └ TwoProcessor.java
 │
 ├ 6
 │  ├ Cache.java
 │  ├ CacheAnnotatedClass.java
 │  └ CacheProcessor.java
 │
 ├ Task2JUnit
 │  ├ InvokeTest.java
...
Разработать аннотацию `@Invoke`, которая может применяться **только к методам**.
Аннотация должна быть доступна **во время выполнения программы**.

Необходимо реализовать обработчик, который с помощью **Reflection API**:

* находит методы, помеченные `@Invoke`
* автоматически вызывает их.
...
* `Invoke` — аннотация
* `InvokeProcessor` — обработчик
* `TestClass` — тестовый класс

### Алгоритм

1. Создать аннотацию `@Invoke`.
2. Указать `Target = METHOD`.
3. Указать `Retention = RUNTIME`.
4. Через Reflection получить список методов класса.
5. Проверить наличие аннотации `@Invoke`.
6. Вызвать найденные методы автоматически.
...
Разработать аннотацию `@Default`, которая может применяться к:

* типам
* полям

Аннотация должна содержать обязательное свойство:

```
value типа Class
```

Необходимо написать обработчик, который выводит **имя класса по умолчанию**.
...
* `Default`
* `DefaultAnnotatedClass`
* `DefaultProcessor`

### Алгоритм

1. Создать аннотацию `@Default`.
2. Добавить свойство `Class value`.
3. Проаннотировать класс.
4. Через Reflection получить значение свойства `value`.
5. Вывести имя класса.
...
Создать аннотацию `@ToString`, которая применяется к **полям или типам**.

Аннотация должна иметь обязательное свойство:
...
Необходимо создать метод, который формирует строковое представление объекта, учитывая только те поля, у которых значение аннотации равно `YES`.
...
1. Создать enum `Mode`.
2. Создать аннотацию `@ToString`.
3. Проаннотировать поля класса `Person`.
4. Через Reflection получить список полей.
5. Проверить значение аннотации.
6. Добавить в строку только поля с `Mode.YES`.
...
Разработать аннотацию `@Validate`, которая может применяться:
...
Обработчик должен выводить список классов, указанных в аннотации.
...
2. Добавить свойство `Class[] value`.
3. Проаннотировать класс.
4. Через Reflection получить список классов.
5. Вывести их имена.
...
Разработать аннотацию `@Two`, которая применяется к типу.

Аннотация должна содержать два свойства:
...
Необходимо создать обработчик, который считывает и выводит значения этих свойств.
...
3. Проаннотировать класс.
4. Через Reflection получить значения свойств.
5. Вывести их.
...
Создать аннотацию `@Cache`, которая применяется к типу.

Аннотация должна иметь свойство:
...
Обработчик должен выводить список кэшируемых областей.
...
2. Добавить свойство `String[] value`.
3. Проаннотировать класс.
4. Через Reflection получить массив строк.
5. Вывести список областей.

---

# Задание 7 — Тестирование (JUnit)

### Формулировка

Необходимо написать тесты с использованием **JUnit**, которые проверяют:

1. корректность работы аннотации `@Invoke`
2. корректность обработки аннотации `@Two`
...
1. Использовать аннотацию `@BeforeEach` для подготовки теста.
2. Вызвать обработчик аннотаций.
...
для проверки выбрасывания исключений.
